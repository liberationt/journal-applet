<template>
  <view class="chineseMain" :style="{ paddingTop: `${navBarHeight + 20}px` }">
    ...
    <template v-if="dictionary.historyList.length">
      ...
      <view class="dicList">
        <view
          v-for="(item, i) in dictionary.historyList"
          :key="i"
          class="dicItem"
          @tap="search(item)"
        >
...
const confirm = (e) => {
  ...
  if (!value) {
    ...
    // search(value)
    Taro.redirectTo({
      url: `../searchChineseResult/index?searchText=${value}`,
    })
    setTimeout(() => {
      dictionary.setHistory(value.replace(/\s+/g, ''))
    }, 200)
  }
}
const search = (value) => {
  ...
  if (pattern.test(value)) {
    Taro.navigateTo({
      url: `../chineseDetail/index?character=${value}`,
    })
  } else {
    Taro.redirectTo({
      url: `../searchChineseResult/index?searchText=${value}`,
