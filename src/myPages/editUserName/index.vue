<template>
  <view class="enterSchool" :style="{ paddingTop: `${navBarHeight + 15}px` }">
    ...
    <view class="identityContent">
      ...
      <view
        :class="['operatBtn', vdata.realNameSelf && 'nextBtn']"
        @tap="vdata.realNameSelf && next()"
        >确定</view
      >
    </view>
  </view>
...
const next = () => {
  personInfo.setUserKey('realNameSelf', vdata.realNameSelf)
  Taro.navigateBack({ delta: 1 })
}
onMounted(() => {
  vdata.realNameSelf = personInfo.userInfo?.realNameSelf || ''
})
</script>
