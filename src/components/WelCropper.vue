...
<template>
  <scroll-view
    :class="[
      ...
      data.cropperData.hidden ? 'hidden' : '',
      props.cropperOptions.type ? 'pzstBg' : '',
    ]"
  >
    <view
      class="cropper_container"
      :class="{ typePzst: props.cropperOptions.type }"
...
onMounted(() => {
  ...
  data.canvasId = `moveCanvas${timerStamp}`
  const device = Taro.getSystemInfoSync()

  const { boxQuestionHeight = '', boxQuestionWidth = '' } = props.cropperOptions
  ...
  setTimeout(() => {
    data.ready = true
    const { src, mode, sizeType, type = false } = props.cropperOptions
    showCropper({
      src,
      mode,
      sizeType,
  ...
  if (src) {
    Taro.getImageInfo({
      ...
      success: function (res) {
        // var w = type ? res.width * 2 : res.width,
        //   h = type ? res.height * 2 : res.height
        var w = type
            ? props.isVertical
              ? res.height * 2
              : res.width * 2
          ...
          h = type
            ...
            ? res.width
            : res.height
        // var w = props.isVertical ? res.height : res.width,
        //   h = props.isVertical ? res.width : res.height
...
function drawLines(moveItems, imageInfo, rotateDegree, callback) {
  ...
  let ctx = Taro.createCanvasContext(data.canvasId)
...
function setupMoveItem(key, changedTouches, imageInfo, callback) {
  ...
  if (changedTouches.length == 1) {
    ...
    let x = touch.clientX
    let y = touch.clientY - navBarHeight - 20
    // 相对画布的点
    x = x - left
    y = y - top
...
function endEvent(e) {
  ...
  let key = e.currentTarget.dataset.key
  console.log(e.changedTouches, 8888)
  setupMoveItem(
    key,
    e.changedTouches,
    {
