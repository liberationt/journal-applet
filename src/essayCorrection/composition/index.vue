<template>
  <view class="compositionMain">
    ...
    <view class="compisitionContent">
      ...
      >
        <view class="mainContent">
          <view v-for="(item, i) in vdata.imgList" :id="`list${i}`" class="list-item" :key="i">
            <view class="item-page">第{{ i + 1 }} 页</view>
            <textarea
              class="textareaText"
              :value="vdata.contentList[i]?.content"
              contenteditable="true"
              auto-height="true"
              placeholder="评论"
              @input="(e) => contentChange(e, i)"
              maxlength="10000"
            ></textarea>
          </view>
        </view>
    ...
    ></GradePopup>
    <!-- 分析内容 -->
    <nut-overlay v-model:visible="vdata.result" :lock-scroll="true" :close-on-click-overlay="false">
...
import { usePerson, useUser } from '@/stores'
...
const contentChange = (e, i) => {
  vdata.contentList[i].content = e.detail.value
}
//开始批改
const begin = () => {
  if (!vdata.title) {
...
//获取图片内容
const getEssayText = (imageUrl) => {
  if (vdata.imgList && vdata.imgList.length > 5) {
    return
  }
  let params = {
    title: vdata.title,
  ...
  getEssayContent(params).then((res: any) => {
    if (res && res.text) {
      vdata.contentList.push({
        content: res.text || '',
        textNumber: res.num || 0,
        title: res.title || '',
      })
      vdata.imgList.push(imageUrl)
...
useDidShow(() => {
  ...
  //   return
  // }
  if (router.params.tempFilePath) {
    getEssayText(router.params.tempFilePath)
  }
  // vdata.imgList.push(router.params.tempFilePath)
})
onMounted(() => {
  vdata.grand = user.userInfo.userClassGrade || 1
  getSelectAll()
})
</script>
<style lang="scss">
